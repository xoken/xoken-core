...
module Network.Xoken.CryptoSpec
    ( spec
    ) where
...
sigMsg :: [ByteString]
sigMsg = [mconcat ["Very secret message ", C.pack (show (i :: Int)), ": 11"] | i <- [0 .. 15]]

sec1 :: SecKeyI
sec1 = fromMaybe (error "Could not decode WIF secret 1") (fromWif bsv strSecret1)

sec2 :: SecKeyI
sec2 = fromMaybe (error "Could not decode WIF secret 2") (fromWif bsv strSecret2)

sec1C :: SecKeyI
sec1C = fromMaybe (error "Could not decode WIF compressed secret 1") (fromWif bsv strSecret1C)

sec2C :: SecKeyI
sec2C = fromMaybe (error "Could not decode WIF compressed secret 2") (fromWif bsv strSecret2C)
...
checkPrivkey = do
    assertBool "Key 1" $ isJust $ fromWif bsv strSecret1
    assertBool "Key 2" $ isJust $ fromWif bsv strSecret2
    assertBool "Key 1C" $ isJust $ fromWif bsv strSecret1C
    assertBool "Key 2C" $ isJust $ fromWif bsv strSecret2C

checkInvalidKey :: Assertion
checkInvalidKey = assertBool "Bad key" $ isNothing $ fromWif bsv strAddressBad

checkMiniKey :: Assertion
checkMiniKey = assertBool "Bad mini key" $ isJust res && fromMiniKey "S6c56bnXQiBjk9mqSYE7ykVQ7NzrRy" == res
  where
    res = do
        bs <- decodeHex "4C7A9640C72DC2099F23715D0C8A0D8A35F8906E3CAB61DD3F78B67BF887C9AB"
...
checkMatchingAddress = do
    assertBool "Key 1" $ Just addr1 == addrToString bsv (pubKeyAddr pub1)
    assertBool "Key 2" $ Just addr2 == addrToString bsv (pubKeyAddr pub2)
    assertBool "Key 1C" $ Just addr1C == addrToString bsv (pubKeyAddr pub1C)
    assertBool "Key 2C" $ Just addr2C == addrToString bsv (pubKeyAddr pub2C)
