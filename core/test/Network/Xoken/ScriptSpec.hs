...
module Network.Xoken.ScriptSpec
    ( spec
    ) where
...
spec = do
    let net = bsv
    describe "bsv scripts" $ props bsv
    describe "bsv scripts" $ props bsv
    describe "multi signatures" $ zipWithM_ (curry mapMulSigVector) mulSigVectors [0 ..]
    describe "signature decoding" $ zipWithM_ (curry (sigDecodeMap net)) scriptSigSignatures [0 ..]
    describe "json serialization" $ do
        it "encodes and decodes script output" $ forAll (arbitraryScriptOutput net) testID
        ...
        it "encodes and decodes siginput" $ forAll (arbitrarySigInput net) (testID . fst)
    describe "script serialization" $ do
        it "encodes and decodes script op" $ property $ forAll arbitraryScriptOp cerealID
        it "encodes and decodes script" $ property $ forAll arbitraryScript cerealID
...
standardSpec net = do
    it "has intToScriptOp . scriptOpToInt identity" $
        property $ forAll arbitraryIntScriptOp $ \i -> intToScriptOp <$> scriptOpToInt i `shouldBe` Right i
    it "has decodeOutput . encodeOutput identity" $
        property $ forAll (arbitraryScriptOutput net) $ \so -> decodeOutput (encodeOutput so) `shouldBe` Right so
    it "has decodeInput . encodeOutput identity" $
        property $ forAll (arbitraryScriptInput net) $ \si -> decodeInput net (encodeInput si) `shouldBe` Right si
    it "can sort multisig scripts" $
        forAll arbitraryMSOutput $ \out ->
            map S.encode (getOutputMulSigKeys (sortMulSig out)) `shouldSatisfy` \xs -> xs == sort xs
    it "can decode inputs with empty signatures" $ do
        ...
        let pk = derivePubKeyI $ wrapSecKey True $ fromJust $ secKey $ B.replicate 32 1
        decodeInput net (Script [OP_0, opPushData $ S.encode pk]) `shouldBe`
            Right (RegularInput (SpendPKHash TxSignatureEmpty pk))
        decodeInput net (Script [OP_0, OP_0]) `shouldBe` Right (RegularInput (SpendMulSig [TxSignatureEmpty]))
...
scriptSpec net =
    when (getNetworkName net == "bsv") $
    it "can verify standard scripts from script_tests.json file" $ do
        xs <- readTestFile "script_tests" :: IO [A.Value]
        let vectorsA = mapMaybe (A.decode . A.encode) xs :: [(String, String, String, String, String)]
            vectorsB = mapMaybe (A.decode . A.encode) xs :: [([Word64], String, String, String, String, String)]
        ...
        forM_ vectors $ \([val], siStr, soStr, flags, res, desc)
         ...
         ->
            unless ("DISABLED" `isInfixOf` flags) $ do
                let _strict =
                        "DERSIG" `isInfixOf` flags || "STRICTENC" `isInfixOf` flags || "NULLDUMMY" `isInfixOf` flags
                    scriptSig = parseScript siStr
                    scriptPubKey = parseScript soStr
                    decodedOutput =
                        fromRight (error $ "Could not decode output: " <> soStr) $ decodeOutputBS scriptPubKey
                    ver = verifyStdInput net (spendTx scriptPubKey 0 scriptSig) 0 decodedOutput (val * 100000000)
                case res of
...
forkIdScriptSpec net =
    ...
    it "can verify scripts from forkid_script_tests.json file" $ do
        ...
        let vectors = mapMaybe (A.decode . A.encode) xs :: [([Word64], String, String, String, String, String)]
        length vectors `shouldBe` 3
        forM_ vectors $ \([valBTC], siStr, soStr, _, res, _) -> do
            let val = valBTC * 100000000
                scriptSig = parseScript siStr
                scriptPubKey = parseScript soStr
                decodedOutput = fromRight (error $ "Could not decode output: " <> soStr) $ decodeOutputBS scriptPubKey
                ver = verifyStdInput net (spendTx scriptPubKey val scriptSig) 0 decodedOutput val
            case res of
                "OK" -> ver `shouldBe` True
                _ -> ver `shouldBe` False

creditTx :: ByteString -> Word64 -> Tx
creditTx scriptPubKey val = Tx 1 [txI] [txO] 0
  where
    txO = TxOut {outValue = val, scriptOutput = scriptPubKey}
    txI = TxIn {prevOutput = nullOutPoint, scriptInput = S.encode $ Script [OP_0, OP_0], txInSequence = maxBound}

spendTx :: ByteString -> Word64 -> ByteString -> Tx
spendTx scriptPubKey val scriptSig = Tx 1 [txI] [txO] 0
  where
...
parseScript str = B.concat $ fromMaybe err $ mapM f $ words str
  where
...
txSigHashSpec net =
    ...
    it "can produce valid sighashes from sighash.json test vectors" $ do
        ...
        let vectors = mapMaybe (A.decode . A.encode) xs :: [(String, String, Int, Integer, String)]
        ...
        forM_ vectors $ \(txStr, scpStr, i, shI, resStr) -> do
            let tx = fromString txStr
                ...
                sh = fromIntegral shI
                res = eitherToMaybe . S.decode . B.reverse =<< decodeHex (cs resStr)
...
txSigHashForkIdSpec net =
    ...
    it "can produce valid sighashes from forkid_sighash.json test vectors" $ do
        ...
        let vectors = mapMaybe (A.decode . A.encode) xs :: [(String, String, Int, Word64, Integer, String)]
...
sigHashSpec net = do
    ...
    it "can correctly show" $ do
        ...
        show (0xabac3344 :: SigHash) `shouldBe` "SigHash " <> show (0xabac3344 :: Word32)
    ...
    it "can decodeTxSig . encode a TxSignature" $
        property $
        forAll (arbitraryTxSignature net) $ \(_, _, ts) -> decodeTxSig net (encodeTxSig ts) `shouldBe` Right ts
    it "can produce the sighash one" $ property $ forAll (arbitraryTx net) $ forAll arbitraryScript . testSigHashOne net
...
mapMulSigVector (v, i) = it name $ runMulSigVector v
  where
...
runMulSigVector (a, ops) = assertBool "multisig vector" $ Just a == b
  where
    ...
    b = do
        ...
        addrToString bsv $ payToScriptAddress d

sigDecodeMap :: Network -> (Text, Int) -> Spec
sigDecodeMap net (_, i) = it ("check signature " ++ show i) func
  where
...
testSigDecode net str =
    let bs = fromJust $ decodeHex str
     ...
     in assertBool (unwords ["Decode failed:", fromLeft (error "Decode did not fail") eitherSig]) $ isRight eitherSig
...
scriptSigSignatures
 ...
 =
    [ "304402205ca6249f43538908151fe67b26d020306c0e59fa206cf9f3ccf641f33357119d02206c82f244d04ac0a48024fb9cc246b66e58598acf206139bdb7b75a2941a2b1e401"
      ...
      -- Strange DER sizes, but in Blockchain. Now invalid as Xoken can only
