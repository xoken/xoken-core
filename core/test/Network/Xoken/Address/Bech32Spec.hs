...
module Network.Xoken.Address.Bech32Spec
    ( spec
    ) where
...
spec = do
    describe "bech32 checksum" $ do
        ...
        it "should have invalid checksum" $ forM_ invalidChecksums testInvalidChecksum
        it "should be a valid address" $ forM_ validAddresses testValidAddress
        it "should be an invalid address" $ forM_ invalidAddresses testInvalidAddress
    ...
    describe "more encoding/decoding cases" $ do
        it "length > 90" $ assert $ isNothing $ bech32Encode "bc" (replicate 82 (word5 (1 :: Word8)))
        it "segwit version bounds" $ assert $ isNothing $ segwitEncode "bc" 17 []
        it "segwit prog len version 0" $ assert $ isNothing $ segwitEncode "bc" 0 (replicate 30 1)
        it "segwit prog len version != 0" $ assert $ isJust $ segwitEncode "bc" 1 (replicate 30 1)
        it "segwit prog len version != 0" $ assert $ isNothing $ segwitEncode "bc" 1 (replicate 41 1)
        it "empty HRP encode" $ assert $ isNothing $ bech32Encode "" []
        it "empty HRP encode" $ assert $ isNothing $ bech32Decode "10a06t8"
        it "hrp lowercased" $ Just "hrp1g9xj8m" `shouldBe` bech32Encode "HRP" []

testValidChecksum :: Bech32 -> Assertion
testValidChecksum checksum =
    case bech32Decode checksum of
        Nothing -> assertFailure (show checksum)
        Just (resultHRP, resultData)
...
testInvalidChecksum checksum = assertBool (show checksum) (isNothing $ bech32Decode checksum)
...
testValidAddress (address, hexscript) = do
    ...
    case segwitDecode hrp address of
        ...
        Just (witver, witprog) -> do
            assertEqual (show address) (decodeHex hexscript) (Just $ segwitScriptPubkey witver witprog)
...
segwitScriptPubkey witver witprog = B.pack $ witver' : fromIntegral (length witprog) : witprog
  where
    witver' =
        if witver == 0
            then 0
