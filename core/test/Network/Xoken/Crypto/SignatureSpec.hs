...
module Network.Xoken.Crypto.SignatureSpec
    ( spec
    ) where
...
spec = do
    describe "signatures" $ do
        it "verify signature" $
            property $ forAll arbitrarySignature $ \(msg, key, sig) -> verifyHashSig msg sig (derivePubKey key)
        it "s component less than half order" $ property $ forAll arbitrarySignature $ isCanonicalHalfOrder . thd3
        it "encoded signature is canonical" $ property $ forAll arbitrarySignature $ testIsCanonical . thd3
        it "encode signature and decode strictly" $
            property $ forAll arbitrarySignature $ (\s -> decodeStrictSig (exportSig s) == Just s) . thd3
...
testIsCanonical sig =
    ...
    (rlen > 1 && BS.index s 4 == 0 && not (testBit (BS.index s 5) 7)) ||
    ...
    (slen > 1 && BS.index s (fromIntegral rlen + 6) == 0 && not (testBit (BS.index s (fromIntegral rlen + 7)) 7))
  where
    s = exportSig sig
