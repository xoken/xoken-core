...
import Network.Xoken.Address (Address(..), pubKeyAddr)
import Network.Xoken.Keys (Fingerprint, KeyIndex, PubKeyI)
import Network.Xoken.Network (VarInt(..), VarString(..), putVarInt)
...
completeSig input (PayScriptHash h)
    ...
    , not (isPayScriptHash decodedScript) = completeSig input decodedScript
...
word8Enum ::
    ...
    -> Either Word8 a
word8Enum n
    | n <= enumWord8 (maxBound :: a) = Right . toEnum $ fromIntegral n
word8Enum n = Left n
