...
{-|
Module      : Network.Xoken.Keys.Extended
...
import Data.Aeson as A (FromJSON, ToJSON, Value(String), parseJSON, toJSON, withText)
...
makeXPrvKey bs = XPrvKey 0 0 0 c k
  where
...
prvSubKey xkey child
    | child >= 0 && child < 0x80000000 = XPrvKey (xPrvDepth xkey + 1) (xPrvFP xkey) child c k
...
pubSubKey xKey child
    | child >= 0 && child < 0x80000000 = XPubKey (xPubDepth xKey + 1) (xPubFP xKey) child c pK
    | otherwise = error "Invalid child derivation index"
  where
...
hardSubKey xkey child
    | child >= 0 && child < 0x80000000 = XPrvKey (xPrvDepth xkey + 1) (xPrvFP xkey) i c k
    | otherwise = error "Invalid child derivation index"
  where
    i = setBit child 31
...
xPubCompatWitnessAddr xkey = pubKeyCompatWitnessAddr (wrapPubKey True (xPubKey xkey))
...
getXPrvKey net = do
    ver <- getWord32be
    unless (ver == getExtSecretPrefix net) $ fail "Get: Invalid version for extended private key"
    XPrvKey <$> getWord8 <*> getWord32be <*> getWord32be <*> S.get <*> getPadPrvKey
...
getXPubKey net = do
    ver <- getWord32be
    unless (ver == getExtPubKeyPrefix net) $ fail "Get: Invalid version for extended public key"
    XPubKey <$> getWord8 <*> getWord32be <*> getWord32be <*> S.get <*> (pubKeyPoint <$> S.get)
...
deriveCompatWitnessAddr k i = (xPubCompatWitnessAddr key, xPubKey key)
  where
    key = pubSubKey k i
...
deriveAddrs k = map f . cycleIndex
  where
    f i =
        let (a, key) = deriveAddr k i
         in (a, key, i)
...
deriveWitnessAddrs k = map f . cycleIndex
  where
    f i =
        let (a, key) = deriveWitnessAddr k i
         in (a, key, i)
...
deriveCompatWitnessAddrs k = map f . cycleIndex
  where
    f i =
        let (a, key) = deriveCompatWitnessAddr k i
         in (a, key, i)
...
(++/) p1 p2 = go id (toGeneric p2) $ toGeneric p1
  where
    go f p =
        case p of
...
instance Show DerivPath where
    showsPrec d p = showParen (d > 10) $ showString "DerivPath " . shows (pathToStr p)
...
instance Show HardPath where
    showsPrec d p = showParen (d > 10) $ showString "HardPath " . shows (pathToStr p)
...
instance Show SoftPath where
    showsPrec d p = showParen (d > 10) $ showString "SoftPath " . shows (pathToStr p)
...
instance FromJSON HardPath where
    parseJSON =
        withText "HardPath" $ \str ->
            case parseHard $ cs str of
                Just p -> return p
                _ -> mzero

instance FromJSON SoftPath where
    parseJSON =
        withText "SoftPath" $ \str ->
            case parseSoft $ cs str of
                Just p -> return p
                _ -> mzero
...
parseBip32PathIndex segment =
    case reads segment of
...
instance Show Bip32PathIndex where
    showsPrec d (Bip32HardIndex i) = showParen (d > 10) $ showString "Bip32HardIndex " . shows i
    showsPrec d (Bip32SoftIndex i) = showParen (d > 10) $ showString "Bip32SoftIndex " . shows i
